...
setlistener("/sim/signals/click", func {
  if (__kbd.shift.getBoolValue()) {
    if (__kbd.ctrl.getBoolValue()) {
      ...
      setprop("sim/messages/copilot", "Wildfire detected!");
    }
  }
});

# Watercanon control for deleting fires
# create timer with 2 second interval
var watercanon = maketimer(2, func

{ 
    if (getprop("/sim/model/mi6/watercanon") > 0)
  {
          var drop_pos = geo.aircraft_position();
          wildfire.resolve_water_drop(drop_pos, 250, 1);
  }     
  }
);
